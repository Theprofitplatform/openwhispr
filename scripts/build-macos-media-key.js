...
if (!swiftTarget) {
  console.error(`[media-key] Unsupported architecture: ${targetArch}`);
  process.exit(1);
}

const projectRoot = path.resolve(__dirname, "..");
const swiftSource = path.join(projectRoot, "resources", "macos-media-key.swift");
const outputDir = path.join(projectRoot, "resources", "bin");
const outputBinary = path.join(outputDir, "macos-media-key");
const hashFile = path.join(outputDir, `.macos-media-key.${targetArch}.hash`);
const moduleCacheDir = path.join(outputDir, ".swift-module-cache");

function log(message) {
  console.log(`[media-key] ${message}`);
}
...
if (!fs.existsSync(swiftSource)) {
  console.error(`[media-key] Swift source not found at ${swiftSource}`);
  process.exit(1);
}
...
if (result.status !== 0) {
  console.error("[media-key] Failed to compile macOS media-key binary.");
...
try {
  ...
  console.warn(`[media-key] Unable to set executable permissions: ${error.message}`);
}

if (!verifyBinaryArch(outputBinary, targetArch)) {
  console.error(
    `[media-key] FATAL: Compiled binary architecture does not match target (${targetArch}).`
  );
  process.exit(1);
}
...
log(`Successfully built macOS media-key binary (${targetArch}).`);
