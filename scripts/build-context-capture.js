...
if (!swiftTarget) {
  console.error(`[context-capture] Unsupported architecture: ${targetArch}`);
  process.exit(1);
}

const projectRoot = path.resolve(__dirname, "..");
const swiftSource = path.join(projectRoot, "resources", "macos-context-capture.swift");
const outputDir = path.join(projectRoot, "resources", "bin");
const outputBinary = path.join(outputDir, "macos-context-capture");
const hashFile = path.join(outputDir, `.macos-context-capture.${targetArch}.hash`);
...
function log(message) {
  console.log(`[context-capture] ${message}`);
}
...
if (!fs.existsSync(swiftSource)) {
  console.error(`[context-capture] Swift source not found at ${swiftSource}`);
  process.exit(1);
}
...
if (result.status !== 0) {
  console.error("[context-capture] Failed to compile macOS context capture binary.");
...
try {
  ...
  console.warn(`[context-capture] Unable to set executable permissions: ${error.message}`);
}

if (!verifyBinaryArch(outputBinary, targetArch)) {
  console.error(
    `[context-capture] FATAL: Compiled binary architecture does not match target (${targetArch}). ` +
...
log(`Successfully built macOS context capture binary (${targetArch}).`);
